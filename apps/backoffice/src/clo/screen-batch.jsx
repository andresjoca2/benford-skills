...
const BatchDetailScreen = ({ batchId, onBack }) => {
  ...
  const activeCompanyRun = activeRuns.find((run) => run.mission === "company_discovery" || run.mission === "companies" || run.mission === "companies_then_people") || null;
...
const BatchPersonas = ({ companies, people, brief, activeRuns = [], onRefresh }) => {
  const companiesKey = (companies || []).map(c => `${c.id}:${c.candidateId}:${c.review}:${c.prospects || 0}`).join("|");
  const peopleKey = (people || []).map(p => `${p.id}:${p.candidateId}:${p.companyId}:${p.review}:${p.score}:${p.email}:${p.linkedinUrl}`).join("|");
  ...
  const selectedPeople = selectedCompany ? rows.filter(p => p.companyId === selectedCompany.id || p.company === selectedCompany.name) : [];
  ...
  const selectedCompanyRun = selectedCompany?.candidateId
    ...
    : null;
  const companyRunActive = Boolean(selectedCompany?.candidateId && selectedCompanyRun);
        ...
        <div className="card emp-list">
          ...
          <div style={{maxHeight:"calc(100vh - 340px)", overflowY:"auto"}}>
            {approvedCompanies.map(company => {
              const companyPeople = rows.filter(p => p.companyId === company.id || p.company === company.name);
        ...
        <div className="card emp-detail">
          ...
          <div style={{padding:"12px 20px 16px", display:"grid", gap:10}}>
            <label className="form-row" style={{margin:0}}>
              <span className="form-label">Feedback para esta empresa</span>
              <textarea
                className="ang-textarea"
                rows={2}
                value={companyFeedback}
                onChange={(event)=>setCompanyFeedback(event.target.value)}
                placeholder="Qué roles buscar o evitar en esta empresa."
              />
            </label>
