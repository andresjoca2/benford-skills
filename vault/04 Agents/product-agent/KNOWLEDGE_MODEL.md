...
### 1.1 Methodology
The primary unit of operational context before documenting a procedure.
A methodology groups the recurring way one office/firm executes procedures that later feed one or more tests.

A methodology can contain:
- shared procedural context
- repeated document logic
- prior SOPs
- auditor-specific variants inside the same methodological line
...
In the current phase, this object is usually not produced yet.

### 5. Auditor-specific SOP
A source-specific or auditor-specific version of a SOP.
This is the main SOP object used in the current phase. It should absorb the useful procedural context, relevant source differences, documentary backing, outputs and open gaps without splitting that knowledge into unnecessary extra files.
...
## Relationships
- A **methodology** provides operational context for one or more **tests / pruebas**.
- A **methodology** can contain multiple auditor executions under the same base logic.
...
- Organize documented knowledge around tests.
- Use methodology as the primary context-loading layer before reading or documenting a procedure.
